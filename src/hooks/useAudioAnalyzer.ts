...
export const useAudioAnalyzer = (audioElement?: HTMLAudioElement | null) => {
  ...
  // Get analyser from AudioEffectsContext (this is the single source of truth)
  const { analyserNode, audioContext } = useAudioEffects();

  useEffect(() => {
    // If no audio element, show demo visualization
    if (!audioElement) {
    ...
    // Use the analyser from AudioEffectsContext if available
    if (analyserNode && audioContext) {
      ...
      // No analyser yet - show demo visualization until audio effects connect
      const mockInterval = setInterval(() => {
        const mockData = new Uint8Array(128);
        mockTimeRef.current += 0.05;
        const time = mockTimeRef.current;
        ...
        for (let i = 0; i < 128; i++) {
          const frequencyFactor = 1 - (i / 128) * 0.6;
          const bassWave = i < 20 ? Math.sin(time * 2.5) * 80 : Math.sin(time * 2.5) * 30;
          const midWave = Math.sin(time * 3.7 + i * 0.15) * 50 * frequencyFactor;
          const highWave = Math.sin(time * 5.3 + i * 0.08) * 30 * frequencyFactor;
          const beatPulse = Math.pow(Math.sin(time * 4), 8) * 60;
  ...
  }, [audioElement, analyserNode, audioContext]);
